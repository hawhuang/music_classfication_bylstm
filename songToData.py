...
def createSpectrogram(filename,newFilename):
	...
	if isMono(rawDataPath+filename):
		command = "cp '{}' 'Data/tmp/{}.mp3'".format(rawDataPath+filename,newFilename)
	else:
		command = "sox '{}' 'Data/tmp/{}.mp3' remix 1,2".format(rawDataPath+filename,newFilename)
	...
	command = "sox 'Data/tmp/{}.mp3' -n spectrogram -Y 200 -X {} -m -r -o '{}.png'".format(newFilename,pixelPerSecond,spectrogramsPath+newFilename)
	p = Popen(command, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True, cwd=currentPath)
	output, errors = p.communicate()
	if errors:
		print errors
	...
	os.remove("Data/tmp/{}.mp3".format(newFilename))
...
def createSpectrogramsFromAudio():
	...
	for index,filename in enumerate(files):
		...
		newFilename = fileGenre+"_"+filename.split(".")[0]
